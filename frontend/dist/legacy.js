...
// Lazy-load collections per tab (avoid loading everything at startup)
const loadedCollections = {};
function loadCollection(name) {
  if (!window.garageDB || !window.garageDB.getAll) return;
  if (loadedCollections[name]) return;
  try {
    const data = window.garageDB.getAll(name) || [];
    switch (name) {
      case 'clients':
        clients = data;
      ...
      case 'items':
        items = data;
        try { (items || []).forEach(ensureItemPricing); } catch (e) {}
        try { rebuildBarcodeIndex(); } catch (e) {}
        break;
      case 'invoices':
        invoices = data;
        break;
    ...
    loadedCollections[name] = true;
    if ((name === 'items' || name === 'invoices') && loadedCollections.items && loadedCollections.invoices) {
      try { migrateLegacyServiceItems(); } catch (e) {}
      try { syncServiceItemsFromItems(); } catch (e) {}
    }
    if (name === 'employees') {
      try { migratePayrollPaymentsToEmployees(); } catch (e) {}
    }
  } catch (e) {}
}
function loadCollections(names) {
  (names || []).forEach(loadCollection);
}
...
try {
  dbReady(() => {
    ...
    try { loadCollection('warehouses'); } catch (e) {}
    try { ensureWarehouses(); } catch (e) {}
  });
} catch(e) {}
...
function showTab(tabName, btn) {
  // Clear reports output when leaving Reports tab
  try {
    if (window.__activeTab === 'reports' && tabName !== 'reports') {
      const reportArea = document.getElementById('reportArea');
      if (reportArea) reportArea.innerHTML = '';
      if (typeof reportPage !== 'undefined') reportPage = 1;
      if (typeof reportClientBalancePage !== 'undefined') reportClientBalancePage = 1;
    }
  } catch(e) {}
  // Close any open overlays/dropdowns when switching tabs
  try { if (typeof closeModal === 'function') closeModal(); } catch(e) {}
  try { if (typeof closeAlertModal === 'function') closeAlertModal(); } catch(e) {}
  try {
    document.querySelectorAll('.client-dropdown, .service-dropdown').forEach(function(dd){
  ...
  if (tabName === 'clients') {
    runOnce('clients', function () {
      loadCollections(['clients','cars','invoices']);
      try { rebuildClientIndex(); } catch (e) {}
      try { rebuildClientCarsSearchIndex(); } catch (e) {}
      renderClients();
    });
  } else if (tabName === 'cars') {
    runOnce('cars', function(){ loadCollections(['cars','clients']); renderCars(); });
  } else if (tabName === 'items') {
    runOnce('items', function () {
      loadCollections(['items','suppliers','warehouses']);
      renderItems();
      renderServiceItems();
    });
    setTimeout(function () {
    ...
    runOnce('invoices', function () {
      loadCollections(['invoices','clients','cars','items']);
      try { rebuildInvoiceClientCache(); } catch (e) {}
      renderInvoices();
      renderInvoiceA();
    });
    ...
    runOnce('suppliers', function(){ loadCollections(['suppliers']); renderSuppliers(); });
  } else if (tabName === 'employees') {
    runOnce('employees', function () {
      loadCollections(['employees']);
      renderEmployees();
      renderPayrollPayments();
    });
  } else if (tabName === 'expenses') {
    runOnce('expenses', function(){ loadCollections(['expenses','invoices']); renderExpenses(); });
    // Always refresh totals when switching to Expenses (income/net depends on invoice state)
    setTimeout(function () {
      try { renderExpenses(); } catch (e) {}
    }, 0);
  } else if (tabName === 'reports') {
    runOnce('reports', function () {
      loadCollections(['clients','invoices','items','employees','expenses','warehouses']);
      try { populateReportClientSelect(); } catch (e) {}
      try { onReportTypeChange(); } catch (e) {}
    });
  } else if (tabName === 'settings') {
    runOnce('settings', function () {
      loadCollections(['users','warehouses']);
      try { refreshWarehouseSettingsUI(); } catch (e) {}
      try { refreshSettingsUser(); } catch (e) {}
    });
  }
}
